...
//   listOf("لَّن", "لَن", "لَنْ", "وَلَن", "أَلَّن", "فَلَن", "وَلَنْ", "فَلَنْ")
  ...
  fun extractSentencesFromIndexdata(
    ...
    if (info.type != "nountype") {
      val extractedSentence =
        if (info.startindex >= 0 && info.endindex <= quranText.length && info.startindex <= info.endindex) {
          quranText.substring(info.startindex, min(info.endindex, quranText.length))
            .trim() // Use min to ensure endindex is within bounds
        } else {
          "Invalid index range"
        }
  ...
  fun addQuranVerseTranslationErab(
    wordInfo: NegationEnt,
    qurantext: String,
    translation: String,
    arIrabTwo: String
    ...
    val dataString =
      "$surah|$ayah|$wordfrom|$wordto|$start|$end|$arabic|$english|$verse|$trans|$types"
    ...
    if (sequence.isNotEmpty()) {
      if (type == "ScenarioRSLT") {
        ...
        if (type == "Scenario 1") {
          val condword =
            sentenceMap["accWordNo"] as Int // Accessing the value for key "accWordNo"
          //   val emphaticVerbWordNo =  sentenceMap["emphaticVerbWordNo"] as Int // Accessing the value for key "emphaticVerbWordNo"
          //   val endword = sentenceMap["endword"] as Int
          val surah = sequence[0].surah
          val ayah = sequence[0].ayah
          val firstWordNo = sequence[0].wordno
...
//if (isEmphaticVerb || isResultVerb) {
        ...
        if (predicateSequence != null) {
          accusativeSentences.add(
            mapOf(
              "type" to "Scenario 1",
              "accWordNo" to accWordNo,
              "quranverse" to quran.get(row.ayah - 1).qurantext,
              "translation" to quran.get(row.ayah - 1).translation,
              "predicateSequence" to predicateSequence
            )
          )
  ...
  fun extractKanaSistersSentences(
    corpusData: List<CorpusEntity>,
    qurantext: String,
    translation: String
  ): List<Map<String, Any>> {
    ...
    for (i in corpusData.indices) {
      ...
      val isValid =
        (tags.contains("ليس") || tags.contains("كود") || tags.contains("صبح") || tags.contains("ظلل") && tags.contains(
          "V"
        ))
