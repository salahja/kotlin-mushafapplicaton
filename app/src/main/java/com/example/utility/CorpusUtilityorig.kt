      ...
      try {
        if (indexstart == 0 || indexstart > 0) {
          ...
          spannableverse.setSpan(
            Constant.sifaspansDark, indexstart, indexend, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE
          )
    ...
    try {
      ...
      try {
        if (indexstart == 0 || indexstart > 0) {
          ...
          spannableverse.setSpan(
            colorSpan, indexstart, indexend, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE
          )
          //   spannableverse.setSpan(new UnderlineSpan(),indexstart, indexend, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        }
      } catch (e: IndexOutOfBoundsException) {
...
//todo 2 188 iza ahudu
  ...
  private fun SifaSpansSetupbysurah(
    ...
    try {
      //  spannableverse = corpusayahWordArrayList[sifaEntity.ayah - 1].spannableverse!!
      spannableverse = corpusayahWordArrayList[sifaEntity.ayah - 1]!![0].spannableverse!!
      //spannableString = SpannableString.valueOf(corpusayahWordArrayList.get(sifaEntity.getAyah() - 1).getSpannableverse());
      try {
        if (indexstart == 0 || indexstart > 0) {
          //   sifaspansDark = getSpancolor(preferences, false);
    ...
    for (nasb in accusativesSentences) {
      ...
      try {
        ...
        try {
          ...
          spannableverse.setSpan(
            Constant.harfinnaspanDark, indexstart, indexend, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE
          )
        } catch (e: IndexOutOfBoundsException) {
        ...
        try {
          ...
          spannableverse.setSpan(
            Constant.harfismspanDark, ismstartindex, ismendindex, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE
          )
        } catch (e: IndexOutOfBoundsException) {
          //     System.out.println(nasb.getSurah() + ":" + nasb.getAyah());
          err.add(nasb.surah.toString() + ":" + nasb.ayah)
        ...
        try {
          ...
          spannableverse.setSpan(
            Constant.harfkhabarspanDark, khabarstart, khabarend, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE
          )
        } catch (e: IndexOutOfBoundsException) {
          //   System.out.println(nasb.getSurah() + ":" + nasb.getAyah());
          err.add(nasb.surah.toString() + ":" + nasb.ayah)
  ...
  private fun MudhafSpansSetup(
    ...
    try {
      ...
      try {
        ...
        println(corpusayahWordArrayList[0]?.get(0)?.corpus!!.ayah)
        println(corpusayahWordArrayList[mudhafen.ayah]?.get(0)!!.corpus!!.ayah)
        spannableverse = corpusayahWordArrayList[mudhafen.ayah]?.get(0)!!.spannableverse!!
      }
  ...
  fun setShartDisplay(
    ...
    if (dark) {
      harfshartspanDark = ForegroundColorSpan(Constant.BYELLOW)
      shartspanDark = ForegroundColorSpan(Constant.BCYAN)
      jawabshartspanDark = ForegroundColorSpan(Color.CYAN)
    } else {
      harfshartspanDark = ForegroundColorSpan(Constant.FORESTGREEN)
      shartspanDark = ForegroundColorSpan(Constant.WMIDNIHTBLUE)
      jawabshartspanDark = ForegroundColorSpan(Constant.WHOTPINK)
    }
        ...
        if (shart.type == "shartonly") {
          ...
          if (harfindex != -1 && shartindex != -1) {
            ...
            shart.spannableVerse!!.setSpan(
              harfshartspanDark, harfindex, harfindex + firstWord.length, 0
            )
...
//    Constant.harfshartspanDark,
        ...
        Toast.makeText(
          ...
          "Surah ID: ${shart.surah}, Ayah ID: ${shart.ayah}",
          Toast.LENGTH_LONG
        ).show()
  ...
  companion object {
    ...
    fun NewSetWordSpanTag(
      ...
      when (tagcounter) {
        1 -> {
          ...
          str.setSpan(
            spanhash[tagone], 0, araone.length, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE
          )
        }
        ...
        2 -> {
          ...
          strtwo.setSpan(
            spanhash[tagtwo], 0, aratwo.length, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE
          )
          strone.setSpan(
            spanhash[tagone], 0, araone.length, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE
          )
        ...
        3 -> {
          ...
          strthree.setSpan(
            spanhash[tagthree], 0, arathree.length, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE
          )
          strtwo.setSpan(
            spanhash[tagtwo], 0, aratwo.length, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE
          )
        ...
        4 -> {
          ...
          strfour.setSpan(
            spanhash[tagfour], 0, arafour.length, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE
          )
          strthree.setSpan(
            spanhash[tagthree], 0, arathree.length, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE
          )
        ...
        5 -> {
          ...
          strfive.setSpan(
            spanhash[tagone], 0, arafive.length, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE
          )
          strfour.setSpan(
            spanhash[tagtwo], 0, arafour.length, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE
          )
          strthree.setSpan(
            spanhash[tagthree], 0, arathree.length, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE
          )
          strtwo.setSpan(
            spanhash[tagfour], 0, aratwo.length, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE
          )
          strone.setSpan(
            spanhash[tagfive], 0, araone.length, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE
          )
    ...
    fun ColorizeRootword(
      ...
      if (tagcounter == 1) {
        ...
        if (tagone == "N" || tagone == "ADJ" || tagone == "PN" || tagone == "V") {
          str = SpannableString(rootword.trim { it <= ' ' })
          str.setSpan(
            spanhash[tagone], 0, rootword.length, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE
          )
        } else if (tagtwo == "N" || tagtwo == "ADJ" || tagtwo == "PN" || tagtwo == "V") {
          str = SpannableString(rootword.trim { it <= ' ' })
          str.setSpan(
            spanhash[tagtwo], 0, rootword.length, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE
          )
        }
        ...
        if (tagone == "N" || tagone == "ADJ" || tagone == "PN" || tagone == "V") {
          ...
          str = SpannableString(rootword.trim { it <= ' ' })
          str.setSpan(
            spanhash[tagthree], 0, rootword.length, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE
          )
        }
    ...
    @JvmStatic
    fun searchForFael(allofQuran: List<QuranEntity>?): List<String> {
      val result = mutableListOf<String>()
      val keyword = "فاعل مرفوع"
      val delimiter = ":"
      val capturedWords = mutableListOf<String>()
      val ayalist = mutableListOf<String>()
      for (pojo in allofQuran!!) {
        //  String ar_irab_two = pojo.getAr_irab_two();

        /*  pojo.ar_irab_two.lines().forEach{ line ->
              if (line.contains(keyword)) {
                  val words = line.substringBefore(delimiter).trim().split("\\s+".toRegex()) // Extract words before delimiter and split
                  capturedWords.addAll(words) // Add all captured words to the list
              }
          }
*/
        ...
        pojo.ar_irab_two.lines().forEach { line ->
          if (line.contains(keyword)) {
            if (line.startsWith("⬤ {")) { // Check if the line starts with a bullet within curly braces
              val wordInBraces = line.substringAfter("{").substringBefore("}")
                .trim() // Extract the word within curly braces
              val datrastring = "${pojo.surah}|${pojo.ayah}|$wordInBraces"
              capturedWords.add(datrastring)
            } else if (line.startsWith("⬤")) { // Check if the line starts with a bullet without curly braces
              val firstWord = line.substringAfter("⬤").trim().split("\\s+".toRegex())
                .firstOrNull() // Extract the first word after the bullet
              if (firstWord != null && firstWord != "⬤") { // Ensure the first word is not null or the bullet
                val datrastring = "${pojo.surah}|${pojo.ayah}|$firstWord"
                capturedWords.add(datrastring)
              }
            } else { // Handle lines without bullets
              val firstWord = line.substringBefore(delimiter).trim().split("\\s+".toRegex())
                .firstOrNull() // Extract the first word before the delimiter
              if (firstWord != null) {
                val datrastring = "${pojo.surah}|${pojo.ayah}|$firstWord"
                capturedWords.add(datrastring)
              }
      ...
      //   wordmatching(capturedWords, quranCorpusList, ayalist)

      return capturedWords
//            return ayalist
    }

    fun updateCorpusWithFael(
      outSideDoerList: List<OutSideDoer>, corpusEntityList: List<CorpusEntity>
    ): List<String> {
      val ayalist = mutableListOf<String>()
      val found = mutableListOf<String>()
      val matchedOutsideDoers = mutableSetOf<OutSideDoer>()
      for (corpusEntity in corpusEntityList) {
        val matchingOutsideDoer = outSideDoerList.find { outsideDoer ->
          outsideDoer.surah == corpusEntity.surah &&
              outsideDoer.ayah == corpusEntity.ayah &&
              outsideDoer.wordno == corpusEntity.wordno
        }
        ...
        val datastring = with(corpusEntity) {
          "$surah?$ayah?$wordno?$wordcount?$araone?$aratwo?$arathree?$arafour?$arafive?" +
              "$tagone?$tagtwo?$tagthree?$tagfour?$tagfive?$rootaraone?$rootaratwo?" +
              "$rootarathree?$rootarafour?$rootarafive?$lemma?$detailsone?$detailstwo?" +
              "$detailsthree?$detailsfour?$detailsfive?$juz?$en?$bn?$ind?$ur?$id"
        }
    ...
    fun updateCorpusWithFaelold(
      ...
      for (outsideDoer in outSideDoerList) {
        val surah = outsideDoer.surah
        val ayah = outsideDoer.ayah
        val wordno = outsideDoer.wordno
        for (list in corpusEntityList) {
          if (list.surah == surah && list.ayah == ayah && list.wordno == wordno) {
            list.detailsone = "${list.detailsone}|FAEL"
            val datastring =
              "$surah?$ayah?$wordno?${list.wordcount}?${list.araone}?${list.aratwo}?${list.arathree}?${list.arafour}?${list.arafive}?" + "${list.tagone}?${list.tagtwo}?${list.tagthree}?${list.tagfour}?${list.tagfive}?${list.rootaraone}?${list.rootaratwo}?" + "${list.rootarathree}?${list.rootarafour}?${list.rootarafive}?${list.lemma}?${list.detailsone}?${list.detailstwo}?" + "${list.detailsthree}?${list.detailsfour}?${list.detailsfive}?${list.juz}?${list.en}?${list.bn}?${list.ind}?${list.ur}?${list.id}"
            ayalist.add(datastring)
          } else {
            val datastring =
              "${list.surah}?${list.ayah}?${list.wordno}?${list.wordcount}?${list.araone}?${list.aratwo}?${list.arathree}?${list.arafour}?${list.arafive}?" + "${list.tagone}?${list.tagtwo}?${list.tagthree}?${list.tagfour}?${list.tagfive}?${list.rootaraone}?${list.rootaratwo}?" + "${list.rootarathree}?${list.rootarafour}?${list.rootarafive}?${list.lemma}?${list.detailsone}?${list.detailstwo}?" + "${list.detailsthree}?${list.detailsfour}?${list.detailsfive}?${list.juz}?${list.en}?${list.bn}?${list.ind}?${list.ur}?${list.id}"
            ayalist.add(datastring)
          }
        }
    ...
    /*
     /*  val corpusEntity = corpusEntityList.find {
          it.surah == surah && it.ayah == ayah && it.wordno == wordno
        }*/

        /*if (corpusEntity != null) {
          corpusEntity.detailsone = "${corpusEntity.detailsone}|FAEL" // Add "|FAEL" to detailsone
        ...
        // Get data from corpusEntity, or use default values if not found
        val datastring = with(corpusEntity ?: CorpusEntity(surah,ayah,wordno,corpusEntity!!.wordcount,corpusEntity.araone,corpusEntity.aratwo,corpusEntity.arathree,corpusEntity.arafour,corpusEntity.arafive,
          corpusEntity.tagone,corpusEntity.tagtwo,corpusEntity.tagthree,corpusEntity.tagfour,corpusEntity.tagfive,corpusEntity.rootaraone,corpusEntity.rootaratwo,corpusEntity.rootarathree,corpusEntity.rootarafour,corpusEntity.rootarafive,corpusEntity.lemma,corpusEntity.detailsone ,
          corpusEntity.detailstwo,corpusEntity.detailsthree,corpusEntity.detailsfour,corpusEntity.detailsfive,
          corpusEntity.en,corpusEntity.bn,corpusEntity.ind,corpusEntity.ur,    corpusEntity.juz,corpusEntity.id)) { // Use default CorpusEntity if null
          "$surah|$ayah|$wordno|$wordcount|$araone|$aratwo|$arathree|$arafour|$arafive|" +
              "$tagone|$tagtwo|$tagthree|$tagfour|$tagfive|$rootaraone|$rootaratwo|" +
              "$rootarathree|$rootarafour|$rootarafive|$lemma|$detailsone|$detailstwo|" +
              "$detailsthree|$detailsfour|$detailsfive|$en|$bn|$ind|$ur|$juz|$id"

        }
        ayalist.add(datastring)*/
     */
    @JvmStatic
    fun searchForFaelwordno(
      allofQuran: List<QuranEntity>?, quranCorpusList: List<CorpusEntity>
    ): List<String> {
      val result = mutableListOf<String>()
      val keyword = "فاعل مرفوع"
      val delimiter = ":"
...
*//*        val strring="أَيْدِي"
                val normalizeWord = normalizeWord(strring)*/
        pojo.ar_irab_two.lines().forEach { line ->
          if (line.contains("⬤")) {
            println("chedk")
          }
          val stringWithoutBullet = line.removePrefix("⬤")
          if (pojo.surah == 2 && pojo.ayah == 16) {
            println()
          }
          if (stringWithoutBullet.contains(keyword)) {
            if (stringWithoutBullet.startsWith("⬤ {")) { // Check if the line starts with a bullet within curly braces
              val wordInBraces = line.substringAfter("{").substringBefore("}")
                .trim() // Extract the word within curly braces
              val datrastring = "${pojo.surah}|${pojo.ayah}|$wordInBraces"
              capturedWords.add(datrastring)
            } else if (stringWithoutBullet.startsWith("⬤")) { // Check if the line starts with a bullet without curly braces
              val firstWord = line.substringAfter("⬤").trim().split("\\s+".toRegex())
                .firstOrNull() // Extract the first word after the bullet
              if (firstWord != null && firstWord != "⬤") { // Ensure the first word is not null or the bullet
                val datrastring = "${pojo.surah}|${pojo.ayah}|$firstWord"
              ...
              val firstWord =
                stringWithoutBullet.substringBefore(delimiter).trim().split("\\s+".toRegex())
                  .firstOrNull() // Extract the first word before the delimiter
              if (firstWord != null) {
                val datrastring = "${pojo.surah}|${pojo.ayah}|$firstWord"
                capturedWords.add(datrastring)
        ...
        // Assuming you have a list of QuranCorpus objects named quranCorpusList
// and a list of extracted data strings named capturedWords


// Now wordNumberList contains the word numbers corresponding to the extracted data


      }


      capturedWords.forEach { extractedData ->
        val (surah, ayah, word) = extractedData.split("|")
        val split = word.split(" ")
        val strippedWord = normalizeWord(split[0])
        //    debugword(quranCorpusList)
        // Find the matching word in the Quran corpus
        val matchingWord = quranCorpusList.find { corpus ->
          corpus.surah == surah.toInt() && corpus.ayah == ayah.toInt() && normalizeWord(
            corpus.araone + corpus.aratwo + corpus.arathree + corpus.arafour + corpus.arafour!!
          ).contains(strippedWord)
        }
        if (surah.toInt() == 2 && ayah.toInt() == 16) {
          println()
        }
        matchingWord?.wordno
        if (matchingWord != null) {
          val datastring =
            "${matchingWord.surah}|${matchingWord.ayah}|${matchingWord.wordno}|${matchingWord.araone + matchingWord.aratwo + matchingWord.arathree + matchingWord.arafour + matchingWord.arafive}"
          ayalist.add(datastring)
        } else {
          val datastring = "$surah|$ayah|0|$word"
          ayalist.add(datastring)
        }
        //  matchingWord?.let { wordNumberList.add(it.wordno) } // Add the word number to the list if found
      }
    ...
    private fun debugword(quranCorpusList: List<CorpusEntity>) {
      for (quran in quranCorpusList.indices) {
        val corpus = quranCorpusList[quran]
        val normal =
          normalizeWord(corpus.araone + corpus.aratwo + corpus.arathree + corpus.arafour + corpus.arafour!!)
        if (corpus.surah == 2 && corpus.ayah == 16 && corpus.wordno == 8) {
          println(normal)
        }
      }
    }

    private fun wordmatching(
      capturedWords: MutableList<String>,
      quranCorpusList: List<CorpusEntity>,
      ayalist: MutableList<String>
    ) {
      val wordNumberList = capturedWords.mapNotNull { extractedData ->
        // Destructure surah, ayah, and word from the extracted data
        val (surah, ayah, word) = extractedData.split("|")
        val split = word.split(" ")
        val strippedWord = extractConsonants(split[0])
        //    val strippedWord = normalizeWord(word)// Strip diacritics from the extracted word
        val combinedara = ""
        // Find the matching word in the Quran corpus
        val matchingWord = quranCorpusList.find { corpus ->
          corpus.surah == surah.toInt() && corpus.ayah == ayah.toInt() &&
        ...
        // Return the word number if found, or null otherwise
        matchingWord?.wordno
        if (matchingWord != null) {
          val datastring =
            "${matchingWord.surah}|${matchingWord.ayah}|${matchingWord.wordno}|${matchingWord.araone + matchingWord.aratwo + matchingWord.arathree + matchingWord.arafour + matchingWord.arafive}"
      ...
      }.toMutableList() // Convert the resulting list to a mutable list if needed
    }

    fun normalizeWord(word: String): String {

      if (word.equals("شَىْءٌ")) {
        return "شيء"
      } else if (word.equals("رَبِّىٓ")) {
        return "ربي"
      } else if (word.equals("إِبْرَٰهِۦمُ")) {
        return "ابراهيم"
      } else if (word.equals("ٱلرَّحْمَٰنُ")) {
        return "الرحمن"
      } else {
        val modifiedWord = replaceSuperscriptAlef(word)
        val mad = modifiedWord.replace("آ", "ءَا")
        val alifhamza = mad.replace("أَ", "ا")
        val alifhamzatwo = alifhamza.replace("أ", "ا")
        val alifhamzabelow = alifhamzatwo.replace("إِ", "ا")
        val result = alifhamzabelow.replace("ٱ", "ا").let { extractConsonants(it) }
        return result
      }
    }

    fun normalizeWordsss(word: String): String {
      val modifiedWord = replaceSuperscriptAlef(word) // Replace superscript alef first
      val result =
        modifiedWord.replace("ٱ", "ا").replace("آ", "ءَا").replace("أَ", "ا").replace("أ", "ا")
          //   .replace("ي","ى")// Replace "ي " with "ي"
          .let { extractConsonants(it) }
      return result
    }


    fun replaceSuperscriptAlef(input: String): String {
      return input.replace("\u0670", "\u0627")
    }

    fun normalizeWords(word: String): String {
      val result = word.replace("ٱ", "ا").let { extractConsonants(it) }
      return result
    }
    ...
    fun searchForTameez(allofQuran: List<QuranEntity>?): List<String> {
      ...
      val regex =
        "\\(([^)]+)\\)(?:\\s+\\w+)*\\s+(تمييز|تمييز\\.|التمييز)" //for capture keyword also
      ...
      val regexmudhaf =
        "\\(([^)]+)\\)(?:\\s+\\w+)*\\s+(?:مضاف إليه.\\.|مضاف إليه|مضاف إليه.)\\s+(\\w+)"
      ...
      for (pojo in allofQuran!!) {
        ...
        while (matcher.find()) {
          ...
          dataString = "${pojo.surah}|${pojo.ayah}|$extracted|"
    ...
    fun highlightKeywords(allofQuran: List<QuranEntity>?) {
      ...
      val keywordGroups = mapOf(
        "shart" to listOf(
          ...
          "شرطية",
          "شرطية.",
          "ظرف متضمن معنى الشرط",
          "وإذا ظرف زمان يتضمن معنى الشرط",
          "ظرف زمان يتضمن معنى الشرط",
          "ولو حرف شرط غير جازم",
          "حرف شرط غير جازم",
      ...
      for (pojo in allofQuran!!) {
        ...
        for ((key, keywords) in keywordGroups) {
          val trie =
            Trie.builder().onlyWholeWordsWhiteSpaceSeparated().addKeywords(keywords).build()
          val emits = trie.parseText(ar_irab_two)

          for (emit in emits) {
            str.setSpan(
              ForegroundColorSpan(
                colorMap[key] ?: Color.BLACK
              ), // Default to black if color not found
              emit.start, emit.start + emit.keyword.length, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE
            )
          }
        }
    ...
    fun HightLightKeyWordold(allofQuran: List<QuranEntity>?) {
      ...
      for (pojo in allofQuran!!) {
        ...
        val haltrie = Trie.builder().onlyWholeWordsWhiteSpaceSeparated().addKeywords(hal).build()
        ...
        val faeltri = Trie.builder().onlyWholeWordsWhiteSpaceSeparated().addKeywords(fael).build()
    ...
    fun QuranWbWSpan(
      ...
      if (tagcounter == 1) {
        ...
        if (faeldetails.contains("|INMUSTATAR")) {
          ...
          str.setSpan(UnderlineSpan(), 0, araone.length + aratwo.length + arathree.length, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE)
          ...
          if (faeldetails.contains("|FAEL")) {
            str =
              SpannableString(araone.trim { it <= ' ' } + aratwo.trim { it <= ' ' } + arathree.trim { it <= ' ' })
            str.setSpan(spanhash[tagone], 0, araone.length, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE)
            str.setSpan(boldSpan, 0, araone.length, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE)
            str.setSpan(
              spanhash[tagtwo],
              araone.length,
              araone.length + aratwo.length,
    ...
    val stringForegroundColorSpanMap: Map<String?, ForegroundColorSpan>
      get() {
        ...
        if (dark) {
          ...
          spanhash["IMPV"] = Constant.lamimpvspanDark // "Imperative lāmprefix(لام الامر)",
          spanhash["PRP"] = Constant.lamtaleelspandDark // "Purpose lāmprefix(لام التعليل)",
          spanhash["SUB"] = Constant.masdariaspanDark // "	Subordinating conjunction(حرف مصدري)",
          ...
          spanhash["CAUS"] = Constant.harfsababiaspanDark // "Particle of cause	(حرف سببية)",
          spanhash["CERT"] = Constant.certainityspanDark // "Particle of certainty	(حرف تحقيق)",
          spanhash["CIRC"] = Constant.halspanDark // "Circumstantial particle	(حرف حال)",
          spanhash["CONJ"] = Constant.particlespanDark // "Coordinating conjunction(حرف عطف)",
          spanhash["COND"] = harfshartspanDark // "Conditional particle(حرف شرط)",
          spanhash["AMD"] = Constant.particlespanDark // "	Amendment particle(حرف استدراك)	",
          spanhash["AVR"] = Constant.particlespanDark // "	Aversion particle	(حرف ردع)",
          spanhash["COM"] = Constant.particlespanDark // "	Comitative particle	(واو المعية)",
          spanhash["EQ"] = Constant.particlespanDark // "	Equalization particle(حرف تسوية)",
          spanhash["EXH"] = Constant.particlespanDark // "	Exhortation particle(حرف تحضيض)",
          spanhash["EXL"] = Constant.particlespanDark // "	Explanation particle(حرف تفصيل)",
          spanhash["EXP"] = Constant.particlespanDark // "	Exceptive particle	(أداة استثناء)",
          spanhash["FUT"] = Constant.particlespanDark // "	Future particle	(حرف استقبال)",
          spanhash["INC"] = Constant.particlespanDark // "	Inceptive particle	(حرف ابتداء)",
          spanhash["INT"] = Constant.particlespanDark // "	Particle of interpretation(حرف تفسير)",
          spanhash["RET"] = Constant.particlespanDark // "	Retraction particle	(حرف اضراب)",
          ...
          spanhash["INL"] = Constant.particlespanDark // "	Quranic initials(	(حروف مقطعة	";
          spanhash["INTG"] =
          ...
          spanhash["PRO"] = Constant.prohibitionspanDark // "	Prohibition particle(حرف نهي)",
          spanhash["REM"] = Constant.resumtionspanDark // "	Resumption particle	(حرف استئنافية)",
          spanhash["RES"] = Constant.restrictivespanDark // "	Restriction particle(أداة حصر)",
          spanhash["RSLT"] =
            Constant.resultparticlespanDark // "Result particle(حرف واقع في جواب الشرط)",
          spanhash["SUP"] = Constant.supplementspoanDark // "	Supplemental particle	(حرف زائد)",
          ...
          spanhash["EMPH"] = Constant.emphspanLight // "Emphatic lām prefix(لام التوكيد) ",
          spanhash["IMPV"] = Constant.lamimpvspanLight // "Imperative lāmprefix(لام الامر)",
          spanhash["PRP"] = Constant.lamtaleelspandLight // "Purpose lāmprefix(لام التعليل)",
          spanhash["SUB"] = Constant.masdariaspanLight // "	Subordinating conjunction(حرف مصدري)",
          ...
          spanhash["CAUS"] = Constant.harfsababiaspanLight // "Particle of cause	(حرف سببية)",
          spanhash["CERT"] = Constant.certainityspanLight // "Particle of certainty	(حرف تحقيق)",
          spanhash["CIRC"] = Constant.particlespanLight // "Circumstantial particle	(حرف حال)",
          spanhash["CONJ"] = Constant.particlespanLight // "Coordinating conjunction(حرف عطف)",
          spanhash["COND"] = Constant.eqspanlight // "Conditional particle(حرف شرط)",
          spanhash["AMD"] = Constant.ammendedparticle // "	Amendment particle(حرف استدراك)	",
          spanhash["AVR"] = Constant.particlespanLight // "	Aversion particle	(حرف ردع)",
          spanhash["COM"] = Constant.particlespanLight // "	Comitative particle	(واو المعية)",
          spanhash["EQ"] = Constant.particlespanLight // "	Equalization particle(حرف تسوية)",
          spanhash["EXH"] = Constant.particlespanLight // "	Exhortation particle(حرف تحضيض)",
          spanhash["EXL"] = Constant.particlespanLight // "	Explanation particle(حرف تفصيل)",
          spanhash["EXP"] = Constant.particlespanLight // "	Exceptive particle	(أداة استثناء)",
          spanhash["FUT"] = Constant.particlespanLight // "	Future particle	(حرف استقبال)",
          spanhash["INC"] = Constant.nasabspanLight // "	Inceptive particle	(حرف ابتداء)",
          spanhash["INT"] = Constant.particlespanLight // "	Particle of interpretation(حرف تفسير)",
          spanhash["RET"] = Constant.particlespanLight // "	Retraction particle	(حرف اضراب)",
          ...
          spanhash["INL"] = Constant.particlespanLight // "	Quranic initials(	(حروف مقطعة	";
          spanhash["INTG"] =
          ...
          spanhash["PRO"] = Constant.prohibitionspanLight // "	Prohibition particle(حرف نهي)",
          spanhash["REM"] = Constant.particlespanLight // "	Resumption particle	(حرف استئنافية)",
          spanhash["RES"] = Constant.restrictivespanLight // "	Restriction particle(أداة حصر)",
          spanhash["RSLT"] =
            Constant.resultparticlespanLight // "Result particle(حرف واقع في جواب الشرط)",
          spanhash["SUP"] = Constant.supplementspanLight // "	Supplemental particle	(حرف زائد)",
          spanhash["SUR"] = Constant.surprisespanLight // "	Surprise particle	(حرف فجاءة)",
        }
        return spanhash
      }
          ...
          str.setSpan(
            ForegroundColorSpan(
              ContextCompat.getColor(
                QuranGrammarApplication.context!!, R.color.midnightblue
              )
    ...
    fun newcomposeWBWCollectiondd(
      quranandCorpusandWbwbySurah: List<QuranEntityCorpusEntityWbwEntity>?, size: Int
    ...
    fun setPastNegation(
      spannableverse: SpannableString, surah_id: Int, ayah_id: Int
    ) {
      val utils = Utils(QuranGrammarApplication.context!!)
        ...
        try {
          ...
          try {
            if (indexstart == 0 || indexstart > 0) {
              ...
              spannableverse.setSpan(
                negationForegroundColorSpan, indexstart, indexend, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE
              )
              //   spannableverse.setSpan(new UnderlineSpan(),indexstart, indexend, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
            }
          } catch (e: IndexOutOfBoundsException) {
...
//todo 2 188 iza ahudu
        ...
        try {
          ...
          try {
            if (indexstart == 0 || indexstart > 0) {
              ...
              spannableverse.setSpan(
                Constant.mudhafspansDark, indexstart, indexend, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE
              )
              //   spannableverse.setSpan(new UnderlineSpan(),indexstart, indexend, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
            }
          } catch (e: IndexOutOfBoundsException) {
    ...
    fun setAbsoluteNegationlatest(
      corpus: java.util.ArrayList<CorpusEntity>?, spannableVerse: SpannableString
    ) {
      ...
      for (i in corpus.indices) {
        ...
        if (isAcc == true && (i > 0 && corpus[i - 1].tagone == "NEG") && // Preceding NEG tag
          ...
          if (validNegIndex != null && !validNegFound) {
            ...
            spannableVerse.setSpan(
              UnderlineSpan(), negStartIndex, phraseEndIndex, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE
            )
          }
        }
      }
    ...
    fun setAbsoluteNegation(
      ...
      for (i in corpus?.indices!!) {
        ...
        if (isAcc!! && // Current word is ACC
          ...
          startIndex =
            occurrences.firstOrNull { (wordNo, _) -> corpus[i - 1].wordno == wordNo }?.second ?: -1
          // Find the start index of the NEG word
          ...
          if (startIndex != -1 && phraseStartIndex != -1) {
            spannableVerse.setSpan(
              UnderlineSpan(), startIndex, phraseEndIndex, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE
            )
          }
        }
      }
    ...
    fun findWordOccurrencesArabic(
      text: String, targetWords: List<String>
    ): List<Pair<Int, Int>> {
    ...
    fun setAbsoluteNegationthree(
      ...
      for (i in corpus?.indices!!) {
        ...
        // Check for the scenario: Noun (ACC), preceding NEG, following P + PRON
        val isAcc = word.detailsone?.contains("ACC")
        if (isAcc == true && i > 0 && corpus[i - 1].tagone == "NEG" // Preceding tag is NEG
          && i < (corpus.size - 2) // Ensure enough following words
          ...
          spannableverse.setSpan(
            UnderlineSpan(), negWordStartIndex, pronWordEndIndex, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE
          )
        }
      }
    }
    ...
    fun findWordIndexByWordNo(
      quranWords: List<String>, corpusWord: CorpusEntity
    ): Int {
      var currentIndex = 0
    ...
    fun twosetAbsoluteNegation(
      ...
      for (i in corpus?.indices!!) {
        ...
        if (isAcc == true && i > 0 && corpus[i - 1].tagone == "NEG" // Preceding tag is NEG
          ...
          // Apply span (underline) to the sentence between start and end index
          spannableverse.setSpan(
            UnderlineSpan(), negWordIndex, phraseEndIndex, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE
          )
        }
      }
    }
    ...
    fun setAbsoluteNegationtwo(
      ...
      for (i in corpus?.indices!!) {
        ...
        // Check for the scenario: Noun (ACC), preceding NEG, following P + PRON
        val isAcc = word.detailsone?.contains("ACC")
        if (isAcc == true && i > 0 && corpus[i - 1].tagone == "NEG" // Check for preceding NEG tag
    ...
    fun findNegTagIndex(
      sentence: String, corpus: ArrayList<CorpusEntity>, negIndex: Int
    ): Int {
      var startIndex = 0
      var currentIndex = 0
    ...
    fun setPresentFromCache(
      spannableverse: SpannableString, presentIndexList: MutableList<List<Int>>?
    ) {
      if (presentIndexList != null) {
        for (indexes in presentIndexList) {
          ...
          if (spannableverse != null) {
            spannableverse.setSpan(
              Constant.sifaspansDark, startIndex, endIndex, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE
            )
          }
        }
      }
    ...
    fun setMausoofSifaFromCache(
      spannableverse: SpannableString, sifaIndexList: MutableList<List<Int>>?
    ) {
    ...
    fun setMudhafFromCache(
      spannableverse: SpannableString, mudhafIndexList: MutableList<List<Int>>?
    ) {
      if (mudhafIndexList != null) {
        for (indexes in mudhafIndexList) {
          ...
          try {
            spannableverse.setSpan(
              colorSpan, startIndex, endIndex, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE
            )
          } catch (e: IndexOutOfBoundsException) {
            println(startIndex)
            println(endIndex)
    ...
    fun setAbsoluteNegationFromCache(
      spannableverse: SpannableString, absoluteNegationIndexes: List<Any>
    ) {
